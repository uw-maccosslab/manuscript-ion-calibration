...
calcIonCalibrationData <- function(df, ref_ion) {
  ...
  # Calculate the mean IonCount for the ref_ion group (MRFA)
  ...
  df_grouped <- df %>%
    ...
    summarize(R_bar = mean(intensity_ratio, na.rm = TRUE),
              ...
              # Set A_a to the pre-calculated mean IonCount for ref_ion (MRFA)
              A_a = A_a_value,
              A_b = mean(IonCount[ion_match != ref_ion], na.rm = TRUE), 
              .groups = 'drop')  # Ensure ungrouping is done right after summarizing
...
# Fig S3: Ultramark + Glu-fib linear regression
